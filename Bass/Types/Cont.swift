...
public extension ContType {
	...
	public func with<I>(_ f: ((I) -> FRC) -> ((IRC) -> FRC)) -> Cont<FRC, I> {
...
public extension ContType {
	public func map(_ f: (FRC) -> FRC) -> Cont<FRC, IRC> {
		return Cont(f • self.run)
	}
	
	public func map<I>(_ f: (IRC) -> I) -> Cont<FRC, I> {
		return Cont { self.run($0 • f) }
	}
	
	public func flatMap<I>(_ fn: (IRC) -> Cont<FRC, I>) -> Cont<FRC, I> {
		return Cont { c in self.run{ fn($0).run(c) } }
	}
	
	public func ap<I, CT: ContType where CT.IRC == (IRC) -> I, CT.FRC == FRC>(_ fn: CT) -> Cont<FRC, I> {
		return Cont { c in fn.run{ g in self.run(c • g) } }
	}
}

/// Alias for `map(f:)`
public func <^> <I, F, CT: ContType where CT.IRC == I, CT.FRC == F>(_ f: (F) -> F, g: CT) -> Cont<F, I> {
	return g.map(f)
}

/// Alias for `map(f:)`
public func <^> <I, I2, F, CT: ContType where CT.IRC == I, CT.FRC == F>(_ f: (I) -> I2, g: CT) -> Cont<F, I2> {
	return g.map(f)
}

/// Alias for `flatMap(g:)`
public func >>- <I, I2, F, CT: ContType where CT.IRC == I, CT.FRC == F>(_ m: CT, _ fn: (I) -> Cont<F, I2>) -> Cont<F, I2> {
	return m.flatMap(fn)
}

/// Alias for `ap(fn:)`
public func <*> <I, I2, F, CT1: ContType, CT2: ContType where CT1.IRC == (I) -> I2, CT1.FRC == F, CT2.IRC == I, CT2.FRC == F>(_ fn: CT1, _ m: CT2) -> Cont<F, I2> {
	return m.ap(fn)
}
